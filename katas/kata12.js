...
We need to complete a function called chooseRecipe(), which will receive three parameters:
    ...
    An array of recipe objects. Each recipe has a name property(string)
    and an ingredient property(array)
...
consider special cases. For example, you do NOT need to worry about cases
...
that will take in one bakery at a time, along with the recipes.ingredients
array to check if the given bakery possesses any of the ingredients from
that recipe.
*/
...
const chooseRecipe = function (bakeryA, bakeryB, recipes) {
  ...
  for (let i = 0; i < recipes.length; i++) {
    let checkA = ingredientCheck(bakeryA, recipes[i].ingredients);
    let checkB = ingredientCheck(bakeryB, recipes[i].ingredients);
    if (checkA && checkB) {
      name = recipes[i].name;
    }
  }
